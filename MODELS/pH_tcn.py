...
s_x, s_y, window = pickle.load(
    open('C:\\Users\\Henry\\PycharmProjects\\MPC project\\pickles\\tcn_needed_Param.pkl', 'rb'))

tcn_mdl = model_defrost(file_path='C:\\Users\\Henry\\PycharmProjects\\MPC '
                                  'project\\MODEL_FROZER\\frozen_models\\tcn_frozen_graph.pb',
                        show_graph=True)
...
def tcn(u_suggest, u, ph, ph_hat):
    ...
    for i in range(window, len(Xs)):
        ...
        Xs[i][(Xs.shape[1] - Ys.shape[1]):] = tcn_mdl(x=tf.constant(Xin))[0][0].numpy()

        Ys[i] = Xs[i][(Xs.shape[1] - Ys.shape[1]):]
    # print(Ysq.shape)
    Ytu = np.ravel(s_y.inverse_transform(Ys))

    return Ytu[window:] #Ys[window:]]
